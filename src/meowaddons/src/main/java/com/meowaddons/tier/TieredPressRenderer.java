...
public class TieredPressRenderer extends KineticBlockEntityRenderer<TieredPressBlockEntity> {
    private static PartialModel headModel(Tier tier) {
        // T1 не перерисовываем, для остальных пока используем оригинал чтобы избежать missing texture (чёрно-фиолетовый)
        // Tiered head текстуры сгенерированы (press_head_t2..t6) но требуют доп. отладки partial baking
