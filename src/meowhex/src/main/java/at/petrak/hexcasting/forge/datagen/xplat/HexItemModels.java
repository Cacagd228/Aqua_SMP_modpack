...
public class HexItemModels extends PaucalItemModelProvider {
    ...
    protected void registerModels() {
        ...
        singleTexture(getPath(HexBlocks.CONJURED_BLOCK),
            ...
            "layer0", ResourceLocation.parse("item/amethyst_shard"));
        ...
        singleTexture("old_staff", ResourceLocation.parse("item/handheld_rod"),
            "layer0", modLoc("item/staff/old"));
        singleTexture("cherry_staff", ResourceLocation.parse("item/handheld_rod"),
        ...
        buildFourVariantGaslight("item/staff/quenched", "item/staff/quenched", (name, path) ->
            singleTexture(path.getPath(), ResourceLocation.parse("item/handheld_rod"),
                "layer0", modLoc(path.getPath())));
        buildFourVariantGaslight(getPath(HexItems.QUENCHED_SHARD), "item/quenched_shard", (name, path) ->
            singleTexture(path.getPath(), ResourceLocation.parse("item/handheld"),
                "layer0", modLoc(path.getPath())));
        ...
        singleTexture(getPath(HexItems.UUID_PIGMENT), ResourceLocation.parse("item/generated"),
            "layer0", modLoc("item/colorizer/uuid"));
        singleTexture(getPath(HexItems.DEFAULT_PIGMENT), ResourceLocation.parse("item/generated"),
            "layer0", modLoc("item/colorizer/uuid"));
        ...
        getBuilder(getPath(HexBlocks.EDIFIED_BUTTON)).parent(
                new ModelFile.UncheckedModelFile(ResourceLocation.parse("block/button_inventory")))
    ...
    private void buildThoughtKnot() {
        var unwritten = singleTexture("thought_knot", ResourceLocation.parse("item/generated"),
            "layer0", modLoc("item/thought_knot"));
        var written = withExistingParent("thought_knot_written", ResourceLocation.parse("item/generated"))
    ...
    private void buildSealableIotaHolder(Item item, String stub, int numVariants) {
        ...
        for (int i = 0; i < numVariants; i++) {
            var plain = i == 0 ? singleTexture(name, ResourceLocation.parse("item/generated"),
                    "layer0", modLoc("item/cad/" + i + "_" + stub + "_empty"))
                    : withExistingParent(name + "_" + i, ResourceLocation.parse("item/generated"))
                    .texture("layer0", modLoc("item/cad/" + i + "_" + stub + "_empty"));
            var unsealed = withExistingParent(name + "_" + i + "_filled", ResourceLocation.parse("item/generated"))
                    .texture("layer0", modLoc("item/cad/" + i + "_" + stub + "_filled"))
                    .texture("layer1", modLoc("item/cad/" + i + "_" + stub + "_filled_overlay"));
            var sealed = withExistingParent(name + "_" + i + "_sealed", ResourceLocation.parse("item/generated"))
    ...
    private void buildStaff(Item item, String name) {
        singleTexture("item/" + getPath(item), ResourceLocation.parse("item/handheld_rod"),
    ...
    private void buildPackagedSpell(Item item, String stub, int numVariants) {
        ...
        for (int i = 0; i < numVariants; i++) {
            ...
            var filled = withExistingParent(name + "_" + i + "_filled", ResourceLocation.parse("item/generated"))
                .texture("layer0", modLoc("item/cad/" + i + "_" + stub))
