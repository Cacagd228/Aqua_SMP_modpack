...
public class MultiCraftingProcessor implements IComponentProcessor {
    ...
    public void setup(Level level, IVariableProvider vars) {
        List<String> names = vars.get("recipes", level.registryAccess()).asStream(level.registryAccess())
            .map(IVariable::asString).collect(Collectors.toList());
    ...
    public IVariable process(Level level, String key) {
        ...
        if (key.equals("output")) {
            return IVariable.wrapList(
                recipes.stream().map(recipe -> recipe.getResultItem(level.registryAccess()))
                    .map(stack -> IVariable.from(stack, level.registryAccess())).collect(Collectors.toList()),
                level.registryAccess());
        }
