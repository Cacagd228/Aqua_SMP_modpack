...
public interface IClientXplatAbstractions {
    ...
    private static IClientXplatAbstractions find() {
        // NB: explicit classloader — ServiceLoader.load(Class) uses the thread context
        // classloader, which is unreliable during FML's parallel event dispatch in dev
        // (ModLauncher transforming loader vs app loader → flaky "not a subtype").
        // The service file and impl live next to this interface, so its own loader is correct.
