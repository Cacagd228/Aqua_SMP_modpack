...
fun drawLineSeq(
    ...
    fun vertex(color: BlockPos, pos: Vec2) =
        buf.addVertex(mat, pos.x, pos.y, z).setColor(color.x, color.y, color.z, a)
    ...
    fun drawCaps(color: BlockPos, point: Vec2, prev: Vec2) {
        ...
        buf = tess.begin(VertexFormat.Mode.TRIANGLE_FAN, DefaultVertexFormat.POSITION_COLOR)
        vertex(color, point)
        for (j in joinSteps downTo 0) {
            val fan = rotate(normal, -Mth.PI * (j.toFloat() / joinSteps))
            buf.addVertex(mat, point.x + fan.x, point.y + fan.y, z).setColor(color.x, color.y, color.z, a)
        }
        BufferUploader.drawWithShader(buf.buildOrThrow())
    }
...
fun drawSpot(mat: Matrix4f, point: Vec2, radius: Float, r: Float, g: Float, b: Float, a: Float) {
    val tess = Tesselator.getInstance()
    val buf = tess.begin(VertexFormat.Mode.TRIANGLE_FAN, DefaultVertexFormat.POSITION_COLOR)
    // https://stackoverflow.com/questions/20394727/gl-triangle-strip-vs-gl-triangle-fan
    // Starting point is the center
    buf.addVertex(mat, point.x, point.y, 1f).setColor(r, g, b, a)
