...
public interface ICircleComponent {
    ...
    static void sfx(BlockPos pos, BlockState bs, Level world, BlockEntityAbstractImpetus impetus, boolean success) {
        sfx(pos, bs, world, impetus, success, true);
    }

    static void sfx(BlockPos pos, BlockState bs, Level world, BlockEntityAbstractImpetus impetus, boolean success,
        boolean withParticles) {
        Vec3 vpos;
        Vec3 vecOutDir;
        ...
        boolean overload = impetus != null && impetus.getExecutionState() != null && impetus.getExecutionState().hasSconce;
        FrozenPigment overloadColor = overload ? new FrozenPigment(new ItemStack(HexItems.DYE_PIGMENTS.get(DyeColor.BLUE)), activator) : colorizer;

        if (withParticles && world instanceof ServerLevel serverLevel) {
            ...
            spray.sprayParticles(serverLevel,
                success ? overloadColor : new FrozenPigment(new ItemStack(HexItems.DYE_PIGMENTS.get(DyeColor.RED)),
                    activator));
        }
