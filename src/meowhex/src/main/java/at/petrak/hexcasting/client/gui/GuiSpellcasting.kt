...
//            emptyList()
    ...
    override fun mouseScrolled(pMouseX: Double, pMouseY: Double, pDeltaX: Double, pDeltaY: Double): Boolean {
        super.mouseScrolled(pMouseX, pMouseY, pDeltaX, pDeltaY)

        val mouseHandler = Minecraft.getInstance().mouseHandler

        if (mouseHandler.accumulatedScroll != 0.0 && sign(pDeltaY) != sign(mouseHandler.accumulatedScroll)) {
            mouseHandler.accumulatedScroll = 0.0
        }

        mouseHandler.accumulatedScroll += pDeltaY
...
//        }
    ...
    fun hexSize(): Float {
        val scaleModifier = Minecraft.getInstance().player!!
            .getAttributeValue(BuiltInRegistries.ATTRIBUTE.wrapAsHolder(HexAttributes.GRID_ZOOM))
