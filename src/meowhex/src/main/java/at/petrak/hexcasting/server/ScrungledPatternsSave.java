...
public class ScrungledPatternsSave extends SavedData {
    ...
    private static ScrungledPatternsSave load(CompoundTag tag, HolderLookup.Provider provider) {
    ...
    public static ScrungledPatternsSave createFromScratch(long seed) {
        var out = new ScrungledPatternsSave(new HashMap<>());
    ...
    private boolean addMissingPatterns(long seed) {
        ...
        for (var key : registry.registryKeySet()) {
            ...
            if (HexUtils.isOfTag(registry, key, HexTags.Actions.PER_WORLD_PATTERN)
                && !this.reverseLookup.containsKey(key)) {
                var scrungledPat = EulerPathFinder.findAltDrawing(entry.prototype(), seed);
                var signature = scrungledPat.anglesSignature();
                var perWorldEntry = new PerWorldEntry(key, scrungledPat.getStartDir());
    ...
    public static ScrungledPatternsSave open(ServerLevel overworld) {
        var save = overworld.getDataStorage().computeIfAbsent(
