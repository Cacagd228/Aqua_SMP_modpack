...
server.tool(
  "list_items",
  "List item IDs from live game registry (use search_items for names). No filter = first N of 21k+",
  {
...
server.tool(
  "list_blocks",
  "List block IDs from live game registry (use search_blocks for names)",
  {
...
server.tool(
  "list_fluids",
  "List fluid IDs from live game registry (use search_fluids for names)",
  {
...
server.tool(
  "get_item_tags",
  "List item tags; with filter returns up to 20 tags with their values (unfiltered = first 50 tags with values, full list is 2400+)",
  {
...
server.tool(
  "get_block_tags",
  "List block tags; with filter returns up to 20 tags with their values",
  {
    filter: z.string().optional().describe("Filter tags by partial name match"),
  },
  async ({ filter }) => {
...
server.tool(
  "get_fluid_tags",
  "List fluid tags; with filter returns up to 20 tags with their values",
  {
    filter: z.string().optional().describe("Filter tags by partial name match"),
  },
  async ({ filter }) => {
...
server.tool(
  "search_items",
  "Search items by ID/name via live client search (returns id, localized name, tags)",
  {
...
server.tool(
  "search_blocks",
  "Search blocks by ID/name via live client search",
  {
    query: z.string().describe("Search query"),
    limit: z.number().optional().default(50).describe("Maximum results"),
  },
...
server.tool(
  "search_fluids",
  "Search fluids by ID/name via live client search",
  {
    query: z.string().describe("Search query"),
    limit: z.number().optional().default(50).describe("Maximum results"),
  },
...
server.tool(
  "add_recipe",
  "Add recipe by writing to live instance kubejs/server_scripts/mcp_generated/mcp_recipes.js + POST /api/reload/server (vanilla + Create + meowaddons tiered machines, tier 1-6)",
  {
    recipe: AnyRecipeSchema,
  },
  async ({ recipe }) => {
    try {
      const res = await client.addRecipe(recipe as any);
      return {
        content: [
          {
            type: "text",
...
server.tool(
  "remove_recipe",
  "Remove recipe by output: appends event.remove to mcp_recipes.js + reload",
  {
